...
if __name__=='__main__':
	fileName = None
	if len(sys.argv) != 2:
		fileName = 'friendsLN.graphml' # Open newly made file
	else:
		fileName = str(sys.argv[1])
	data = extract_friend_data(fileName) # extract the data on the file
	create_plot_data(data)
def create_plot_data(friendData):
	sortedArray = []
	output = []
	output = open('friendsLN.txt','w') # This puts the information in a .txt file
	counter = 0 
